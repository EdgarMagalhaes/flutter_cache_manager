...
const statusCodesNewFile = [HttpStatus.ok, HttpStatus.accepted, HttpStatus.notFound];
const statusCodesFileNotChanged = [HttpStatus.notModified];

class WebHelper {
  WebHelper(this._store, FileService? fileFetcher) : _memCache = {}, fileFetcher = fileFetcher ?? HttpFileService();
  ...
  Future<void> _downloadOrAddToQueue(String url, String key, Map<String, String>? authHeaders) async {
  ...
  Stream<FileResponse> _updateFile(String url, String key, {Map<String, String>? authHeaders}) async* {
    ...
    cacheObject =
        cacheObject == null
            ? CacheObject(url, key: key, validTill: clock.now(), relativePath: '${const Uuid().v1()}.file')
  ...
  Stream<FileResponse> _manageResponse(CacheObject cacheObject, FileServiceResponse response) async* {
    ...
    yield FileInfo(file, FileSource.Online, newCacheObject.validTill, newCacheObject.url, statusCode: response.statusCode);
  }
